...
namespace Market_State_Calculator
{
    public sealed class Market_State_Calculator : Indicator
    {
        ...
        protected override void OnUpdate(UpdateArgs args)
        {
            ...
            double slope8EMA = CalculateSlope(this.emaMini, this.SlopePeriodMini);
            double slope50EMA = CalculateSlope(this.emaFast, this.SlopePeriodFast);
            double slope200EMA = CalculateSlope(this.emaSlow, this.SlopePeriodSlow);

            // Calculate the range of slopes over the lookback period
            double slopeRange8 = CalculateSlopeRange(this.emaMini, this.SlopePeriodMini, this.lookbackPeriod);
            double slopeRange50 = CalculateSlopeRange(this.emaFast, this.SlopePeriodFast, this.lookbackPeriod);
            double slopeRange200 = CalculateSlopeRange(this.emaSlow, this.SlopePeriodSlow, this.lookbackPeriod);

            // Define the dynamic thresholds
            double neutralThresholdMini = slopeRange8 * 0.05;
            double neutralThresholdFast = slopeRange50 * 0.05;
            double neutralThresholdSlow = slopeRange200 * 0.05;
            double doubleThreshold = slopeRange8 * 0.3;

            // Calculate scores for each EMA slope using dynamic thresholds
            int scoreMiniEMA = slope8EMA > doubleThreshold ? 2 : slope8EMA < -doubleThreshold ? -2 : slope8EMA > neutralThresholdMini ? 1 : slope8EMA < -neutralThresholdMini ? -1 : 0;
            int scoreFastEMA = slope50EMA > neutralThresholdFast ? 1 : slope50EMA < -neutralThresholdFast ? -1 : 0;
            int scoreSlowEMA = slope200EMA > neutralThresholdSlow ? 1 : slope200EMA < -neutralThresholdSlow ? -1 : 0;
